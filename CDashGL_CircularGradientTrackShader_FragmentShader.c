   ...
   if(start_deg > end_deg)
   {
       ...
       if(track_angle_deg >= 0.0)
       {
           ...
           if(abs(track_angle_deg) > 180.0)
           {
               ...
               else
               {
                   //  根据角度信息计算出百分比
                   float per;
                   if(angle_deg >= start_deg) per = ((360.0 - angle_deg) + end_deg) * 1.0f / ((360.0 - start_deg) + end_deg);
                   else                       per = (end_deg - angle_deg) * 1.0f / ((360.0 - start_deg) + end_deg);
                   outColor = texture(texture1, tex_color_pos) * (in_blend_color * (per)) ;
               }
           }
           //  当为小弧度一侧
           else
       ...
       else
       {
           ...
           if(abs(track_angle_deg) > 180.0)
           {
               ...
               {
                   //  根据角度信息计算出百分比
                   float per = (angle_deg - end_deg) * 1.0f / (start_deg - end_deg);
                   outColor = texture(texture1, tex_color_pos) * (in_blend_color * per);
               }
               else
               {
   ...
   else
   {
       ...
       if(track_angle_deg >= 0.0)
       {
           ...
           if(abs(track_angle_deg) > 180.0)
           {
               ...
               {
                   //  根据角度信息计算出百分比
                   float per = (angle_deg - start_deg) * 1.0f / (end_deg - start_deg);
                   outColor = texture(texture1, tex_color_pos) * (in_blend_color * (1.0 - per));
               }
               else
               {
                   outColor = vec4(0.0, 0.0, 0.0, 0.0);
       ...
       else
       {
           ...
           if(abs(track_angle_deg) > 180.0)
           {
               ...
               else
               {
                   //  根据角度信息计算出百分比
                   float per;
                   if(angle_deg >= start_deg) per = ((360.0 - angle_deg) + start_deg) * 1.0f / ((360.0 - end_deg) + start_deg);
                   else                       per = (start_deg - angle_deg) * 1.0f / ((360.0 - end_deg) + start_deg);
                   outColor = texture(texture1, tex_color_pos) * (in_blend_color * (1.0 - per));
               }
           }
           //  当为小弧度一侧
